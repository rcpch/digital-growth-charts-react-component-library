...
describe('All tests relate to rendering the centile lines in the height centile chart with no data.', () => {
    ...
    const measurementMethods = [
        ...
        MeasurementMethods.weight,
        MeasurementMethods.bmi,
        MeasurementMethods.ofc,
    ];
    ...
    it('should render 2nd centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.queryByTestId('reference-1-centile-2-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.queryByTestId('reference-2-centile-2-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.queryByTestId('reference-3-centile-2-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 9th centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-9-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-9-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-9-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 25th centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-25-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-25-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-25-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 50th centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-50-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-50-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-50-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 75th centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-75-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-75-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-75-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 91st centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-91-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-91-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-91-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 98th centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-98-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-98-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-98-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
    ...
    it('should render 99.6th centile', () => {
        measurementMethods.forEach((item, index) => {
            ...
            render(chart);
            expect(
                screen.getByTestId('reference-1-centile-99.6-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-2-centile-99.6-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
            expect(
                screen.getByTestId('reference-3-centile-99.6-measurement-' + props.measurementMethod),
            ).toBeInTheDocument();
        });
    });
});
...
describe('All tests relate to rendering the text in the height centile chart for an older boy.', () => {
    ...
    it('should render UK-WHO reference attribution label text correctly.', () => {
        render(<CentileChart {...props} />);
        expect(
            screen.queryByText('WHO Child Growth Standards, UK 1990 reference data, reanalysed 2009'),
        ).toBeInTheDocument();
    });
...
describe('All tests relate to rendering the text in the height centile chart for an older girl.', () => {
    ...
    it('should render Turner reference attribution label text correctly.', () => {
        ...
        render(<CentileChart {...props} />);
        expect(
            screen.queryByText('UK Turner reference data, 1985. Lyon, Preece and Grant (1985).'),
        ).toBeInTheDocument();
    });

    it('renders centile labels when labels are enabled and have room at the standard chart size.', () => {
        render(<CentileChart {...props} width={1000} height={800} />);
        expect(screen.getAllByTestId(/^curve-label-centile-.*-right$/)).toHaveLength(9);
        expect([0, 9]).toContain(screen.queryAllByTestId(/^curve-label-centile-.*-left$/).length);
    });
});
