...
function isCDCReferences(data: UKWHOReferences | CDCReferences): data is CDCReferences {
    return 'cdc_infant' in data || 'cdc_child' in data;
}
    ...
    if (
        ...
        centileReferenceData.forEach((referenceData: UKWHOReferences | CDCReferences, index) => {
            ...
            if (reference === 'uk-who' && isUKWHOReferences(referenceData)) {
                ...
                centiles = referenceData.cdc_infant || referenceData.cdc_child;
                lowercentiles =
                    lowerCentileReferenceData[index].cdc_infant || lowerCentileReferenceData[index].cdc_child;
                uppercentiles =
                    upperCentileReferenceData[index].cdc_infant || upperCentileReferenceData[index].cdc_child;
            } else {
