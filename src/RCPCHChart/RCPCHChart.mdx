...
- `logoVariant?: 'top' | 'bottom' | 'legend';`: allows the RCPCH logo to be de-emphasised in favour of a less prominent `powered by RCPCH` statement at the foot of the chart.
- `allowDuplicates?: boolean;`: enable if `measurements` contains duplicates (same `measurementMethod` performed on the same day) - this is signposted in the tooltips of any duplicates. Defaults to false.
...
- `centileTextStyle?: Pick<TextStyle, 'name' | 'size' | 'weight' | 'colour'>;` // centile and BMI SDS label text style

Curve labels default to `axisStyle.tickLabelTextStyle.colour` (black if unset), independently of the curve stroke colours. Override their colour with `customThemeStyles.centileStyle.centileTextStyle.colour`:

```tsx
const customThemeStyles = {
    centileStyle: {
        centileTextStyle: { colour: '#006699' },
    },
};
```

This applies to both centile labels and the SDS reference labels on UK-WHO BMI charts, including SVG exports. Omit `colour` to follow the axis tick-label colour.
