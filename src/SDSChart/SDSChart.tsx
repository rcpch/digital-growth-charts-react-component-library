...
const SDSChart: React.FC<SDSChartProps> = ({
    ...
    const legendMeasurements = useMemo(
        ...
        () => selectedMeasurementMethods(childMeasurements, styles),
    ...
    const midParentalHeightSDSData = useMemo(
        () => generateMidParentalHeightSDSData(domains, midParentalHeightData),
        [domains, midParentalHeightData],
    );
    ...
    if (childMeasurements[measurementMethod].length > 0) {
        const lowestYMeasurement = childMeasurements[measurementMethod].reduce((a, b) =>
            a.plottable_data.sds_data.corrected_decimal_age_data.y <
            b.plottable_data.sds_data.corrected_decimal_age_data.y
                ? a
                : b,
        );
        const highestYMeasurement = childMeasurements[measurementMethod].reduce((a, b) =>
            a.plottable_data.sds_data.corrected_decimal_age_data.y >
            b.plottable_data.sds_data.corrected_decimal_age_data.y
                ? a
                : b,
                                    ...
                                    measurementTypeItem.measurementTypeData.length > 0 && (
                                        ...
                                        >
                                            <VictoryLine
                                                ...
                                                x={(datum) =>
                                                    datum.plottable_data.sds_data.chronological_decimal_age_data.x
                                                }
                                                y={(datum) =>
                                                    datum.plottable_data.sds_data.chronological_decimal_age_data.y
                                                }
                                                labelComponent={<VictoryLabel renderInPortal dx={20} title="height" />}
                ...
                <ButtonContainer>
                    ...
                    {enableExport ? (
                        <ShareButtonWrapper>
                            ...
                            <CopiedLabel $active={active} onAnimationEnd={labelFadeEnd}>
                                Copied!
                            </CopiedLabel>
                        </ShareButtonWrapper>
                    ) : (
