...
- [ ] **R4 - Complete the Node 24 migration** - Resolve and close GitHub issue #227 after validating Clear compoeted build, test, Storybook and publication workflows.
...
- [x] **R8 - Stop warning on legacy data with no actionable issue** - Follow-up to [#217](https://github.com/rcpch/digital-growth-charts-react-component-library/issues/217#issuecomment-5582600992): a chart viewer cannot act on a permanent, non-dismissible warning about pre-provenance persisted data. `ProvenanceWarningBanner` now renders only when at least one issue is `unknown` or `mismatch`; legacy-only datasets render without a warning. Point filtering and mismatch suppression are unchanged. The provenance contract doc in `digital-growth-charts-documentation` is updated to match.
